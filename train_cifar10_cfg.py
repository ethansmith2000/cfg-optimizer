...
from cfg_adamv1 import CFGAdamV1
...
optimizer = CFGAdamV1(net.parameters(), lr=args.lr, base_grad_alpha=args.base_grad_alpha)
...
def train(epoch):
    ...
    for batch_idx, (inputs, targets) in enumerate(trainloader):
        ...
        # sample a value that is not the true target
        # Generate random targets and ensure they're different from actual targets
        fake_targets = torch.randint(0, num_classes - 1, targets.size(), device=device)

        # Add 1 to fake_targets where they're >= actual targets to avoid equality
        fake_targets = torch.where(fake_targets == targets, fake_targets + 1, fake_targets)

        # Ensure values stay within valid range
        fake_targets = fake_targets % num_classes

        # negative example
        with torch.cuda.amp.autocast(enabled=use_amp):
            outputs = net(inputs)
            fake_loss = criterion(outputs, fake_targets)
            print(f"Fake loss: {fake_loss}")
        scaler.scale(fake_loss).backward()

        neg_grad_norm = 0
        for p in net.parameters():
            neg_grad_norm += p.grad.norm()
        print(f"Negative grad norm: {neg_grad_norm}")

        optimizer.load_negative_gradient()
        # optimizer.zero_grad(set_to_none=True)
        ...
        pos_grad_norm = 0
        for p in net.parameters():
            pos_grad_norm += p.grad.norm()
        print(f"Positive grad norm: {pos_grad_norm}")
